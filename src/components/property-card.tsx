...
export function PropertyCard({
  ...
  return (
    ...
    >
      <img
        src={image}
        className={`${isPopup ? "max-h-28 min-h-28 min-w-48 max-w-48" : "h-52 w-72"}`}
      />
      <div className="flex flex-col gap-2 p-3">
        <div className="flex items-center justify-between">
          <p className={`text-${isPopup ? "xs" : "sm"} text-landing`}>
            {title}
          </p>
          <div className="flex items-center text-primary">
            <span
              className={`${isPopup ? "text-xs" : "text-sm"} font-bold text-landing`}
            >
              {price}
            </span>
